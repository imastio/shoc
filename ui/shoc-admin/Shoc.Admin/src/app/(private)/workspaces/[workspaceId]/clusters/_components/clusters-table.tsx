...
export default function ClustersTable({ workspaceId, loading = false }: any) {
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            key: 'id',
            width: '100px',
            render: (id: string, row: any) => <Link prefetch={false} href={`/workspaces/${row.workspaceId}/clusters/${id}`}><Button><RightOutlined /></Button></Link>
        }
    ], []);
    ...
    const load = useCallback(async () => {
        ...
        const result = await withToken((token: string) => selfClient(ClustersClient).getAllExtended(token, workspaceId))
