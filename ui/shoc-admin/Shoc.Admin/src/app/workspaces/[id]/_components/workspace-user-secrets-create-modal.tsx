...
export default function WorkspaceUserSecretsCreateModal({workspaceId, userId, open, onClose = () => {}, onSuccess = () => {}}: any){
    ...
    const onSubmit = async (values: any) => {
        ...
        const result = await withToken((token: string) => selfClient(UserSecretsClient).create(token, workspaceId, userId, input));
    ...
    return (
        <Modal
            ...
            onOk={() => form.submit()}
        >
            <StandardAlert style={{ marginBottom: "8px" }} message={`Could not add the secret for the workspace`} errors={errors} optional={true} />
            <Form
                form={form}
                preserve={false}
                onFinish={onSubmit}
