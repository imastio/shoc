...
export default function WorkspaceSecretsUpdateModal({workspaceId, existing, open, onClose = () => {}, onSuccess = () => {}}: any){
    ...
    const onSubmit = async (values: any) => {
        ...
        const result = await withToken((token: string) => selfClient(SecretsClient).updateById(token, workspaceId, existing?.id, input));
    ...
    return (
        <Modal
            ...
            onOk={() => form.submit()}
        >
            <StandardAlert style={{ marginBottom: "8px" }} message={`Could not edit the selected secret`} errors={errors} optional={true} />
            <Form
                disabled={progress}
                form={form}
                preserve={false}
