...
export default function SecretUpdateDialog({ item, workspaceId, open, trigger, onClose, onSuccess }: DialogProps) {
  ...
  const onOk: SubmitHandler<FieldValues> = async (values) => {
    ...
    const { data, errors } = await rpc('secret/workspace-secrets/updateById', {
  ...
  return <Dialog open={open} onOpenChange={onOpenChangeWrapper} modal>
    ...
    <DialogContent className="w-4/5 md:w-1/2">
      <DialogHeader>
        <DialogTitle>{intl.formatMessage({ id: 'secrets.update.title.workspaceSecret' })}</DialogTitle>
        <DialogDescription>
          {intl.formatMessage({ id: 'secrets.update.notice.workspaceSecret' })}
        </DialogDescription>
      </DialogHeader>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        autoFocus
                        placeholder={intl.formatMessage({ id: 'secrets.placeholders.name.workspaceSecret' })}
                        type="text"
                        autoCapitalize="none"
                        autoComplete="off"
