...
export default function WorkspaceInvitationDeleteDialog({ item, open, onClose, trigger, onSuccess }: DialogProps) {
    ...
    const onOk = async () => {
        ...
        setErrors([]);
        setProgress(true);
        const { data, errors } = await rpc('workspace/user-workspace-invitations/deleteById', { workspaceId: item.workspaceId, id: item.id });
        setProgress(false);
