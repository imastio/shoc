...
export default function SecretDeleteDialog({ item, open, onClose, trigger, onSuccess }: DialogProps) {
    ...
    const onOk = async () => {
        ...
        setErrors([]);
        setProgress(true);
        const { data, errors } = await rpc('secret/workspace-secrets/deleteById', { workspaceId: item.workspaceId, id: item.id });
        setProgress(false);
