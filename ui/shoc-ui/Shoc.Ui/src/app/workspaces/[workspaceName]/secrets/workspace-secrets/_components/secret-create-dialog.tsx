...
export default function SecretCreateDialog({ workspaceId, open, trigger, onClose, onSuccess }: DialogProps) {
  ...
  const onOk: SubmitHandler<FieldValues> = async (values) => {
    ...
    const { data, errors } = await rpc('secret/workspace-secrets/create', {
  ...
  return <Dialog open={open} onOpenChange={onOpenChangeWrapper} modal>
    ...
    <DialogContent className="w-4/5 md:w-1/2">
      <DialogHeader>
        <DialogTitle>{intl.formatMessage({ id: 'secrets.create.title.workspaceSecret' })}</DialogTitle>
        <DialogDescription>
          {intl.formatMessage({ id: 'secrets.create.notice.workspaceSecret' })}
        </DialogDescription>
      </DialogHeader>
