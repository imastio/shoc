...
export default function SecretValueUpdateDialog({ item, workspaceId, open, trigger, onClose, onSuccess }: DialogProps) {
  ...
  const onOk: SubmitHandler<FieldValues> = async (values) => {
    ...
    const { data, errors } = await rpc('secret/workspace-secrets/updateValueById', {
  ...
  return <Dialog open={open} onOpenChange={onOpenChangeWrapper} modal>
    ...
    <DialogContent className="w-4/5 md:w-1/2">
      <DialogHeader>
        <DialogTitle>{intl.formatMessage({ id: 'secrets.updateValue.title.workspaceSecret' })}</DialogTitle>
        <DialogDescription>
          {intl.formatMessage({ id: 'secrets.updateValue.notice.workspaceSecret' })}
        </DialogDescription>
      </DialogHeader>
